...
/**
 * Plugin for OpenStego which implements the MSB algorithm of steganography for 
 * seglo art project
 */
public class MSBPlugin extends LSBPlugin
{
    /**
     * Constant for Namespace to use for this plugin
     */
    public final static String NAMESPACE = "MSB";
    ...
    public MSBPlugin()
    {
        LabelUtil.addNamespace(NAMESPACE, "net.sourceforge.openstego.resource.MSBPluginLabels");
    }
            ...
            throws OpenStegoException
    {
        ...
        try
        {
            ...
            msbOS = new MSBOutputStream(image, msg.length, msgFileName, this.config);
            msbOS.write(msg);
            msbOS.close();

            return ImageUtil.imageToByteArray(msbOS.getImage(), stegoFileName, this);
        }
    ...
    {
        MSBInputStream msbIS = null;

        msbIS = new MSBInputStream(ImageUtil.byteArrayToImage(stegoData, stegoFileName), this.config);
        return msbIS.getDataHeader().getFileName();
    }
    ...
    public byte[] extractData(byte[] stegoData, String stegoFileName, byte[] origSigData) throws OpenStegoException
    {
        ...
        {
            msbIS = new MSBInputStream(ImageUtil.byteArrayToImage(stegoData, stegoFileName), this.config);
            header = msbIS.getDataHeader();
            data = new byte[header.getDataLength()];

            bytesRead = msbIS.read(data, 0, data.length);
